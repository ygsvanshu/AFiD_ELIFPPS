---
title: Numerical methods
nav_order: 5
---

# Numerical methods

This section containing the numerics is organized as following:

## General numerical details

- [Time integration](time-integration.md)
- [Trilinear Interpolation](interpolation.md)
- [Stability](stability.md)

## Eulerian fluid phase
- [Spatial discretization](spatial-discretization.md)
- [Advective terms](advective-terms.md)
- [Diffusive terms](diffusive-terms.md)
- [Pressure solver](pressure-solver.md)
- [Boundary conditions](boundary-conditions.md)

## Lagrangian particle phase
- [Slip correction](slip-correction.md)
- [Asynchronous spawn and exit](asynchronous.md)
    ...
    if (.not.periodic(1)) then
        do kc = xstart(3),xend(3)
            do jc = xstart(2),xend(2)
                ...
                if (btyp.eq.DIRICHLET)  cfbcxs(jc,kc,1) =  0.0
                if (btyp.eq.NEUMANN)    cfbcxs(jc,kc,1) =  1.0
                if (btyp.eq.SOMMERFELD) cfbcxs(jc,kc,1) =  (bval*al*dt)/((bval*al*dt) + dxc(1  ))
                call VxBcXe(jc,kc,btyp,bval)
                if (btyp.eq.DIRICHLET)  cfbcxs(jc,kc,1) =  0.0
                if (btyp.eq.NEUMANN)    cfbcxs(jc,kc,1) =  1.0
                if (btyp.eq.SOMMERFELD) cfbcxs(jc,kc,1) =  (bval*al*dt)/((bval*al*dt) + dxc(nxm))
            end do
        end do
    end if
    !! For y-pencils
    if (.not.periodic(2)) then
        do kc = ystart(3),yend(3)
            do ic = ystart(1),yend(1)
                ...
                if (btyp.eq.DIRICHLET)  cfbcys(ic,kc,1) = -1.0
                if (btyp.eq.NEUMANN)    cfbcys(ic,kc,1) =  1.0
                if (btyp.eq.SOMMERFELD) cfbcys(ic,kc,1) =  (bval*al*dt)/((bval*al*dt) + dym(1  ))
                call VxBcYe(ic,kc,btyp,bval)
                if (btyp.eq.DIRICHLET)  cfbcye(ic,kc,1) = -1.0
                if (btyp.eq.NEUMANN)    cfbcye(ic,kc,1) =  1.0
                if (btyp.eq.SOMMERFELD) cfbcye(ic,kc,1) =  (bval*al*dt)/((bval*al*dt) + dym(ny ))
            end do
        end do
    end if
    !! For z-pencils
    if (.not.periodic(3)) then
        do jc = zstart(2),zend(2)
            do ic = zstart(1),zend(1)
                ...
                if (btyp.eq.DIRICHLET)  cfbczs(ic,jc,1) = -1.0
                if (btyp.eq.NEUMANN)    cfbczs(ic,jc,1) =  1.0
                if (btyp.eq.SOMMERFELD) cfbczs(ic,jc,1) =  (bval*al*dt)/((bval*al*dt) + dzm(1  ))
                call VxBcZe(ic,jc,btyp,bval)
                if (btyp.eq.DIRICHLET)  cfbcze(ic,jc,1) = -1.0
                if (btyp.eq.NEUMANN)    cfbcze(ic,jc,1) =  1.0
                if (btyp.eq.SOMMERFELD) cfbcze(ic,jc,1) =  (bval*al*dt)/((bval*al*dt) + dzm(nz ))
            end do
        end do
    end if
    ...
    if (.not.periodic(1)) then
        do kc = xstart(3)-lvlhalo,xend(3)+lvlhalo
            do jc = xstart(2)-lvlhalo,xend(2)+lvlhalo
                ...
                if (btypxs(jc,kc,1).eq.SOMMERFELD) bvalxs(jc,kc,1) = bvalxs(jc,kc,1)*al*dt/dxc(1  )
                call VxBcXe(jc,kc,btypxe(jc,kc,1), bvalxe(jc,kc,1))
                !!! Store this value needed for imposing B.C.s later
                if (btypxe(jc,kc,1).eq.SOMMERFELD) bvalxe(jc,kc,1) = bvalxe(jc,kc,1)*al*dt/dxc(nxm)
            end do
        end do
    end if
    !! For y-boundaries
        ...
        if (xstart(2).eq.1) then
            do kc = xstart(3)-lvlhalo,xend(3)+lvlhalo
                do ic = xstart(1),xend(1)+lvlhalo
                    ...
                    !!! Store this value needed for imposing B.C.s later
                    if (btypys(ic,kc,1).eq.SOMMERFELD) bvalys(ic,kc,1) = bvalys(ic,kc,1)*al*dt/dym(1  )
                end do
            end do
        end if
        if (xend(2).eq.nym) then
            do kc = xstart(3)-lvlhalo,xend(3)+lvlhalo
                do ic = xstart(1),xend(1)+lvlhalo
                    ...
                    !!! Store this value needed for imposing B.C.s later
                    if (btypye(ic,kc,1).eq.SOMMERFELD) bvalye(ic,kc,1) = bvalye(ic,kc,1)*al*dt/dym(ny )
                end do
            end do
        end if
    end if
        ...
        if (xstart(3).eq.1) then
            do jc = xstart(2)-lvlhalo,xend(2)+lvlhalo
                do ic = xstart(1),xend(1)+lvlhalo
                    ...
                    !!! Store this value needed for imposing B.C.s later
                    if (btypzs(ic,jc,1).eq.SOMMERFELD) bvalzs(ic,jc,1) = bvalzs(ic,jc,1)*al*dt/dzm(1  )
                end do
            end do
        end if
        if (xend(3).eq.nzm) then
            do jc = xstart(2)-lvlhalo,xend(2)+lvlhalo
                do ic = xstart(1),xend(1)+lvlhalo
                    ...
                    !!! Store this value needed for imposing B.C.s later
                    if (btypze(ic,jc,1).eq.SOMMERFELD) bvalze(ic,jc,1) = bvalze(ic,jc,1)*al*dt/dzm(nz )
                end do
            end do
        end if
    end if
    ...
    if (.not.periodic(1)) then
        do kc = xstart(3),xend(3)
            do jc = xstart(2),xend(2)
                ...
                if (btypxs(jc,kc,1).eq.SOMMERFELD) rhx(2  ,jc,kc) = rhx(2  ,jc,kc) + beta*am1si(2  )*(bvalxs(jc,kc,1)*(vx(2,jc,kc) - vx(1,jc,kc))/(1.0 + bvalxs(jc,kc,1)))
                ...
                if (btypxe(jc,kc,1).eq.SOMMERFELD) rhx(nxm,jc,kc) = rhx(nxm,jc,kc) + beta*ap1si(nxm)*(bvalxe(jc,kc,1)*(vx(nxm,jc,kc) - vx(nx,jc,kc))/(1.0 + bvalxe(jc,kc,1)))
            end do
        end do
    end if
        ...
        if (xstart(2).eq.1) then
            do kc = xstart(3),xend(3)
                do ic = xstart(1),xend(1)
                    ...
                    if (btypys(ic,kc,1).eq.SOMMERFELD) rhx(ic,1  ,kc) = rhx(ic,1  ,kc) + beta*am2cj(1  )*(bvalys(ic,kc,1)*(vx(ic,1,kc) - vx(ic,0,kc))/(1.0 + bvalys(ic,kc,1)))
                end do
            end do
        end if
        if (xend(2).eq.nym) then
            do kc = xstart(3),xend(3)
                do ic = xstart(1),xend(1)
                    ...
                    if (btypye(ic,kc,1).eq.SOMMERFELD) rhx(ic,nym,kc) = rhx(ic,nym,kc) + beta*ap2cj(nym)*(bvalye(ic,kc,1)*(vx(ic,nym,kc) - vx(ic,ny,kc))/(1.0 + bvalye(ic,kc,1)))
                end do
            end do
        end if
    end if
        ...
        if (xstart(3).eq.1) then
            do jc = xstart(2),xend(2)
                do ic = xstart(1),xend(1)
                    ...
                    if (btypzs(ic,jc,1).eq.SOMMERFELD) rhx(ic,jc,1  ) = rhx(ic,jc,1  ) + beta*am3ck(1  )*(bvalzs(ic,jc,1)*(vx(ic,jc,1) - vx(ic,jc,0))/(1.0 + bvalzs(ic,jc,1)))
                end do
            end do
        end if
        if (xend(3).eq.nzm) then
            do jc = xstart(2),xend(2)
                do ic = xstart(1),xend(1)
                    ...
                    if (btypze(ic,jc,1).eq.SOMMERFELD) rhx(ic,jc,nzm) = rhx(ic,jc,nzm) + beta*ap3ck(nzm)*(bvalze(ic,jc,1)*(vx(ic,jc,nzm) - vx(ic,jc,nz))/(1.0 + bvalze(ic,jc,1)))
                end do
            end do
        end if
    end if

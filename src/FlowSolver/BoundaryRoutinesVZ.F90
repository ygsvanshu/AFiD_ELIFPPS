    ...
    if (.not.periodic(1)) then
        do kc = xstart(3),xend(3)
            do jc = xstart(2),xend(2)
                ...
                if (btyp.eq.DIRICHLET)  cfbcxs(jc,kc,3) = -1.0
                if (btyp.eq.NEUMANN)    cfbcxs(jc,kc,3) =  1.0
                if (btyp.eq.SOMMERFELD) cfbcxs(jc,kc,3) =  (bval*al*dt)/((bval*al*dt) + dxm(1  ))
                call VzBcXe(jc,kc,btyp,bval)
                if (btyp.eq.DIRICHLET)  cfbcxe(jc,kc,3) = -1.0
                if (btyp.eq.NEUMANN)    cfbcxe(jc,kc,3) =  1.0
                if (btyp.eq.SOMMERFELD) cfbcxe(jc,kc,3) =  (bval*al*dt)/((bval*al*dt) + dxm(nx ))
            end do
        end do
    end if
    !! For y-pencils
    if (.not.periodic(2)) then
        do kc = ystart(3),yend(3)
            do ic = ystart(1),yend(1)
                ...
                if (btyp.eq.DIRICHLET)  cfbcys(ic,kc,3) = -1.0
                if (btyp.eq.NEUMANN)    cfbcys(ic,kc,3) =  1.0
                if (btyp.eq.SOMMERFELD) cfbcys(ic,kc,3) =  (bval*al*dt)/((bval*al*dt) + dym(1  ))
                call VzBcYe(ic,kc,btyp,bval)
                if (btyp.eq.DIRICHLET)  cfbcye(ic,kc,3) = -1.0
                if (btyp.eq.NEUMANN)    cfbcye(ic,kc,3) =  1.0
                if (btyp.eq.SOMMERFELD) cfbcye(ic,kc,3) =  (bval*al*dt)/((bval*al*dt) + dym(ny ))
            end do
        end do
    end if
    !! For z-pencils
    if (.not.periodic(3)) then
        do jc = zstart(2),zend(2)
            do ic = zstart(1),zend(1)
                ...
                if (btyp.eq.DIRICHLET)  cfbczs(ic,jc,3) =  0.0
                if (btyp.eq.NEUMANN)    cfbczs(ic,jc,3) =  1.0
                if (btyp.eq.SOMMERFELD) cfbczs(ic,jc,3) =  (bval*al*dt)/((bval*al*dt) + dzc(1  ))
                call VzBcZe(ic,jc,btyp,bval)
                if (btyp.eq.DIRICHLET)  cfbcze(ic,jc,3) =  0.0
                if (btyp.eq.NEUMANN)    cfbcze(ic,jc,3) =  1.0
                if (btyp.eq.SOMMERFELD) cfbcze(ic,jc,3) =  (bval*al*dt)/((bval*al*dt) + dzc(nzm))
            end do
        end do
    end if
    ...
    if (.not.periodic(1)) then
        do kc = xstart(3),xend(3)+lvlhalo
            do jc = xstart(2)-lvlhalo,xend(2)+lvlhalo
                ...
                if (btypxs(jc,kc,3).eq.SOMMERFELD) bvalxs(jc,kc,3) = bvalxs(jc,kc,3)*al*dt/dxm(1  )
                call VzBcXe(jc,kc,btypxe(jc,kc,3), bvalxe(jc,kc,3))
                !!! Store this value needed for imposing B.C.s later
                if (btypxe(jc,kc,3).eq.SOMMERFELD) bvalxe(jc,kc,3) = bvalxe(jc,kc,3)*al*dt/dxm(nx )
            end do
        end do
    end if
    !! For y-boundaries
        ...
        if (xstart(2).eq.1) then
            do kc = xstart(3),xend(3)+lvlhalo
                do ic = xstart(1)-lvlhalo,xend(1)+lvlhalo
                    ...
                    !!! Store this value needed for imposing B.C.s later
                    if (btypys(ic,kc,3).eq.SOMMERFELD) bvalys(ic,kc,3) = bvalys(ic,kc,3)*al*dt/dym(1  )
                end do
            end do
        end if
        if (xend(2).eq.nym) then
            do kc = xstart(3),xend(3)+lvlhalo
                do ic = xstart(1)-lvlhalo,xend(1)+lvlhalo
                    ...
                    !!! Store this value needed for imposing B.C.s later
                    if (btypye(ic,kc,3).eq.SOMMERFELD) bvalye(ic,kc,3) = bvalye(ic,kc,3)*al*dt/dym(ny )
                end do
            end do
        end if
    end if
        ...
        if (xstart(3).eq.1) then
            do jc = xstart(2)-lvlhalo,xend(2)+lvlhalo
                do ic = xstart(1)-lvlhalo,xend(1)+lvlhalo
                    ...
                    !!! Store this value needed for imposing B.C.s later
                    if (btypzs(ic,jc,3).eq.SOMMERFELD) bvalzs(ic,jc,3) = bvalzs(ic,jc,3)*al*dt/dzc(1  )
                end do
            end do
        end if
        if (xend(3).eq.nzm) then
            do jc = xstart(2)-lvlhalo,xend(2)+lvlhalo
                do ic = xstart(1)-lvlhalo,xend(1)+lvlhalo
                    ...
                    !!! Store this value needed for imposing B.C.s later
                    if (btypze(ic,jc,3).eq.SOMMERFELD) bvalze(ic,jc,3) = bvalze(ic,jc,3)*al*dt/dzc(nzm)
                end do
            end do
        end if
    end if
    ...
    if (.not.periodic(1)) then
        do kc = xstart(3),xend(3)
            do jc = xstart(2),xend(2)
                ...
                if (btypxs(jc,kc,3).eq.SOMMERFELD) rhx(1  ,jc,kc) = rhx(1  ,jc,kc) + beta*am1ci(1  )*(bvalxs(jc,kc,3)*(vz(1,jc,kc) - vz(0,jc,kc))/(1.0 + bvalxs(jc,kc,3)))
                ...
                if (btypxe(jc,kc,3).eq.SOMMERFELD) rhx(nxm,jc,kc) = rhx(nxm,jc,kc) + beta*ap1ci(nxm)*(bvalxe(jc,kc,3)*(vz(nxm,jc,kc) - vz(nx,jc,kc))/(1.0 + bvalxe(jc,kc,3)))
            end do
        end do
    end if
        ...
        if (xstart(2).eq.1) then
            do kc = xstart(3),xend(3)
                do ic = xstart(1),xend(1)
                    ...
                    if (btypys(ic,kc,3).eq.SOMMERFELD) rhx(ic,1  ,kc) = rhx(ic,1  ,kc) + beta*am2cj(1  )*(bvalys(ic,kc,3)*(vz(ic,1,kc) - vz(ic,0,kc))/(1.0 + bvalys(ic,kc,3)))
                end do
            end do
        end if
        if (xend(2).eq.nym) then
            do kc = xstart(3),xend(3)
                do ic = xstart(1),xend(1)
                    ...
                    if (btypye(ic,kc,3).eq.SOMMERFELD) rhx(ic,nym,kc) = rhx(ic,nym,kc) + beta*ap2cj(nym)*(bvalye(ic,kc,3)*(vz(ic,nym,kc) - vz(ic,ny,kc))/(1.0 + bvalye(ic,kc,3)))
                end do
            end do
        end if
    end if
        ...
        if (xstart(3).eq.1) then
            do jc = xstart(2),xend(2)
                do ic = xstart(1),xend(1)
                    ...
                    if (btypzs(ic,jc,3).eq.SOMMERFELD) rhx(ic,jc,2  ) = rhx(ic,jc,2  ) + beta*am3sk(2  )*(bvalzs(ic,jc,3)*(vz(ic,jc,2) - vz(ic,jc,1))/(1.0 + bvalzs(ic,jc,3)))
                end do
            end do
        end if
        if (xend(3).eq.nzm) then
            do jc = xstart(2),xend(2)
                do ic = xstart(1),xend(1)
                    ...
                    if (btypze(ic,jc,3).eq.SOMMERFELD) rhx(ic,jc,nzm) = rhx(ic,jc,nzm) + beta*ap3sk(nzm)*(bvalze(ic,jc,3)*(vz(ic,jc,nzm) - vz(ic,jc,nz))/(1.0 + bvalze(ic,jc,3)))
                end do
            end do
        end if
    end if
